...
На загрузку
$$\frac{500 * 10^6}{24 * 60} * 3.84 = 1333 \text{ гбит/c}$$

На отправку требуется
$$\frac{500 * 10^6}{24 * 60} * 2.56 = 888 \text{ гбит/c}$$

Вычеслим RPS.

- Для видео и аудио файлов будем получать
$$\frac{500 * 10^6}{24 * 60 * 2} =  173612\text{ запросов в секунду.}$$
- Для генерации приглашений будем получать
...
|---	|---	|---	|
|  Загрузка видео 	|  173 612 	| 41.3 |
|  Загрузка аудио 	|  173 612	| 16 | 
...
Итого будем иметь, с учетом сохранения 250 тыс. ежедневных записей (половина числа пользователей, имеющих платную подписку)
$$284 * \frac{250 * 10^3}{2}  = 8.46\text{ тбайт/день записей.}$$
...
Для поддержания последовательности сообщений и обеспечение конкуретного доступа будем во время коференции записывать каждый чат в таблицу,
а по завершению трансляции сохранять все чаты в файл и отправлять в S-3 на храненение. После сохранения в текстовый файл произодится удаление ненудных картежей из базы данных.
...
3. PostgreSQL: 1 Master, 2 Slaves
4. Tarantool: 1 master x 1 replica

# 7. Список серверов

Frontend:

Фронтенд сервера больщую часть времени будут заниматься балансировкой, поэтому можем взять только 16 CPU

$$\frac{354400 * 3}{16 * 128}  = 519 \text{ шт.}$$

Сетевой трафик:

$$\frac{1333}{519} = 3 \text{ гбит/c }$$

Для стабильности работы и расширяемости возьмем 10 гбит/c.
...
$$\frac{354400 * 3 * 57.3}{64 * 512}  = 1860 \text{ шт.}$$

Postgres:

По 5 серверов на регион:
1 master, 2 slaves, 2 replica. Reserve реплецирует оригинал и находится в другом дата центре, находящися в том же регионе воизбежание потери данных при авариях.

Tarantool:

По 2 сервера на регион:
1 master, 1 replica, 1 reserve. Reserve находится в другом ДЦ на случай аварий, при этом репликации не проиходит.

Конфигурация серверов

|  Применение 	|  CPU | RAM | Мак. объем сетевого трафика на сервер, гбит/c | Количество |
|---	|---	|--- |--- |--- |
| Бекенд сервера | 64 | 512 | 10 | 1860 |
| Фронтенд сервера | 16 | 128 | 10 | 519 |
| Postgres | разное | разное | 20 | 25 | 
| Tarantool | разное | разное | 20 | 10 |

На S-3 хранилища требуется закладывать не менее

$$8.46 * 2 =  16.92 \text{ тбайт/день}$$

с учетом двойного запаса.

Распределение серверов по зонам. Определяем по IP адресу организатора конференции.

Северная Америка: 
|  Применение 	| Количество |
|---	|--- |
| Бекенд сервера | 836 |
| Фронтенд сервера | 234 |

|  Применение 	|  CPU | RAM | Мак. объем сетевого трафика на сервер, гбит/c | Количество |
|---	|---	|--- |--- |--- |
| Postgres | 32 | 256 | 20 | 5 | 
| Tarantool | 32 | 64 | 20 | 2 |
...
|---	|--- |
| Бекенд сервера | 372 |
| Фронтенд сервера | 104 |

|  Применение 	|  CPU | RAM | Мак. объем сетевого трафика на сервер, гбит/c | Количество |
|---	|---	|--- |--- |--- |
| Postgres | 16 | 128 | 20 | 5 | 
| Tarantool | 16 | 32 | 20 | 2 |
